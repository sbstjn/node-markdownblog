 ...
 *
 * (c) 2012 Sebastian Müller <semu.mp>
 * MIT license
 */

/**
 * Required needed modules and define variables
 **/
var fs = require('fs')
 , crypto = require('crypto')
 , y = require('yamlish')
 , md = require('markdown').markdown
 , app = {'meta': {'general': {}, 'opengraph': {}}, 'conf': {'dir': null, 'pagesdir': null}, 'pagesFiles': {}, 'files': [], 'hidden': [], 'articles': [], 'tags': {}, 'default': {}, 'mapping': {}, 'data': {}, 'md': require('markdown').markdown, 'admin': [], 'userIsAdmin': false, 'maxID': 3141}
 ...
 * @param mixed value
 **/
exports.setMeta = function(name, value) { this.meta.general[name] = value; };

/** 
 * Set opengraph meta information
 * @param string name
 * @param mixed value
 **/
exports.setOG = function(name, value) { this.meta.opengraph[name] = value; };
 ...
 **/
exports.isAdmin = function() { app.userIsAdmin = true; };

/** 
 * Set current user to default user
 **/
exports.isUser = function() { app.userIsAdmin = false; };
...
exports.updateArticle = function(id, data) {
  if (this.hidden[id]) {
    var curItem = this.hidden[id]; } 
  else {
    var curItem = this.data[id]; }
  
  var curFile = this.conf.dir + '/' + curItem.file

  var fd = fs.openSync(curFile, 'w+', 0666);
  fs.writeSync(fd, data);
  fs.closeSync(fd);
  
  this.index(this.conf.dir);
};
...
exports.createNewArticle = function(name) {
  ...
  var newFile = this.conf.dir + '/' + curID + '-' + slug + '.md';
 ...
 * @return array
 **/
exports.jadeData = function(data) {
...
exports.scanForArticleID = function(id) {
  var articleList = fs.readdirSync(this.conf.dir).sort();
  var file = null;
...
exports.index = function(dir) {
  ...
  this.hidden = [];
  this.tags = [];
  this.files = fs.readdirSync(this.conf.dir).sort();
...
exports.pages = function(dir) {
  this.conf.pagesdir = dir;
  this.pagesFiles = fs.readdirSync(this.conf.pagesdir).sort();
  this.processPages();
};
...
exports.parseFile = function(b) {
  var c=(fs.readFileSync(this.conf.dir+'/'+b)+'').split('*/'),d=c[0].substr(3),e=c[1],f=y.decode(d),g=[],h=f.Sources||{};
  ...
  return {
    ...
    'date': m,
    'md': (fs.readFileSync(this.conf.dir+'/'+b)),
...
exports.parseFilePage = function(b) {
  var c=(fs.readFileSync(this.conf.pagesdir+'/'+b)+'').split('*/'),d=c[0].substr(3),e=c[1],f=y.decode(d),g=[],h=f.Sources||{};
  var name=(/Title:(.*)/.exec(d)[1]+'').replace('Title:','').replace(/^\s\s*/, '').replace(/\s\s*$/, '');
...
exports.getArticle = function(id, showPreview) {
  if (showPreview == true && this.hidden[id]) {
    return this.hidden[id]; }
  if (!this.data[id] || this.data[id].hidden == true) {
    return null; }
  else {
    return this.data[id]; }
};
...
exports.processPages = function() {
  for (a=this.pagesFiles.length;a--;) {
    var page = this.parseFilePage(this.pagesFiles[a]);
...
exports.processArticles = function() {
  for (a=this.files.length;a--;) {
    var article = this.parseFile(this.files[a]);
    ...
    if (article.hidden) {
      this.hidden[article.id] = article;
      continue; 
    } else {
      this.data[article.id] = article;
      delete article.content;
